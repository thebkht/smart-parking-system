...
- `POST /map` + `GET /map` store and retrieve parking lot quad polygon layout
- `GET /sessions` returns per-spot occupancy session history
- `POST /park` accepts a driver photo, runs SIFT localization, returns a session ID
- `GET /find/{session_id}` returns spot location and corner coordinates for Find My Car
