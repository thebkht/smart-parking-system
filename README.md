...
The project is complete for the PRD when all five flows run end-to-end without mocks:

1. **Owner setup** → photos uploaded → SfM runs → layout saved to DB → polygon map displayed with label-correction step
2. **Edge pipeline** → `detect.py` posts occupancy JSON → `GET /status` returns `{ spots, confidence, timestamp }` → live map reads `response.spots` and colors polygons correctly
3. **Find My Car** → driver photo → `POST /park` → real session ID → `GET /find/{session_id}` → correct spot highlighted in amber
4. **Contracts aligned** → docs, backend routes, and frontend fetch paths all agree; no leftover PKLot/CNR or v3 references
5. **Basic verification** → happy-path tests pass for `/park`, `/find/{id}`, `/map`/`/layout`, and frontend response parsing

Core deliverables at that point:

- Core edge occupancy pipeline ✓
- Backend 7-endpoint surface ✓ (pending `/park` fix)
- Web app 3 screens ✓ (pending parser + wiring fixes)
- Find My Car flow ✓ (pending end-to-end wiring)
- Owner setup flow ✓ (pending SfM invocation + label-correction UI)
- Supporting docs/runbook aligned to v6 ✓ (pending cleanup)

**Open question — React Native:** The PRD names it explicitly. If the team treats web-only as acceptable for the final submission and demo, the project is done without mobile. If the PRD is read literally, mobile remains outstanding until an Expo wrapper exists or the scope is formally reduced in docs.

---

## Remaining Gap Checklist

### 🔴 Must fix — project is not done without these

These five items are what stand between the current state and a fully working end-to-end system.

- [ ] **Fix duplicate `POST /park` handler** (`backend/main.py`) — stub returning `501` sits before the real multipart handler; blocks the entire Find My Car flow — [@abdusattormv](https://github.com/abdusattormv)
- [ ] **Fix `GET /status` response parser** (`frontend/src/App.jsx`) — frontend reads the whole object as spot states instead of `response.spots`; live map coloring and free/occupied count are both broken — [@mirzayv](https://github.com/mirzayv)
- [ ] **Wire Find My Car frontend end-to-end** — replace fake session IDs and random-spot fallback with real `POST /park` → store `session_id` → `GET /find/{session_id}` → amber highlight flow — [@mirzayv](https://github.com/mirzayv)
- [ ] **Wire SfM into backend** — `ml/sfm_layout.py` exists but `backend/main.py` never invokes it; `POST /layout` must run SfM on uploaded photos rather than storing a pre-built payload — [@abdusattormv](https://github.com/abdusattormv) + [@thebkht](https://github.com/thebkht)
- [ ] **Add happy-path backend tests** — `/park`, `/find/{session_id}`, `/sessions`, and `/map` are not covered in `tests/test_backend.py`; these must pass before submission — [@abdusattormv](https://github.com/abdusattormv)

### 🟡 Should fix for PRD compliance

### 🟡 Should fix for PRD compliance

- [ ] **Fix `POST /layout` fetch path** — align frontend call to the canonical backend route name once [@abdusattormv](https://github.com/abdusattormv) confirms it; owner setup screen is currently broken without this — [@mirzayv](https://github.com/mirzayv)
- [ ] **Owner setup: polygon label-correction step** — PRD owner flow requires editing/relabelling spot polygons after layout generation; `frontend/src/App.jsx:347` only previews the map with no edit workflow — [@mirzayv](https://github.com/mirzayv)
- [ ] **Reference-photo management for Find My Car** — PRD implies per-spot stored references uploaded and persisted in DB; backend currently reads a fixed local sample folder — [@abdusattormv](https://github.com/abdusattormv)
- [ ] **Resolve doc inconsistencies** — `docs/final-artifact-summary.md`, `docs/final-runbook.md`, and `backend/README.md` still disagree on routes and architecture in places; normalise to v6 before report submission — [@thebkht](https://github.com/thebkht)
- [ ] **Write frontend README** — `frontend/README.md` is still the default Vite template; add setup instructions, env vars, screen descriptions, and backend connection guide — [@mirzayv](https://github.com/mirzayv)
- [ ] **Add frontend tests for PRD screens and API contracts** — no frontend tests exist for owner setup, live map, or Find My Car flows — [@mirzayv](https://github.com/mirzayv)

### 🟢 Nice to have

- [ ] **End-to-end smoke-test script** — a single runnable script that walks the full PRD path: owner setup → map persistence → edge updates → live map → park/find; currently no such script exists
- [ ] **React Native / Expo wrapper** — formally completes the PRD mobile deliverable; deprioritised given timeline — [@mirzayv](https://github.com/mirzayv)
- [ ] **Auth and session ownership model** — no auth or multi-user handling present; acceptable for a class demo but absent from a fuller product interpretation
- [ ] **Verified green runtime** — a confirmed `make test` + `make backend` + `make edge` run in a clean env would close the operational verification gap
