...
The owner uploads reference photos of the lot, an SfM pass computes a
bird's-eye-view layout and extracts quadrilateral spot polygons, and the
owner publishes the result via \texttt{POST /map}. This is the one-time
configuration step that every other screen depends on. The screenshot in
Figure~\ref{fig:ui-owner} shows the web flow driving a sample placeholder
layout (\texttt{spot\_source: placeholder\_grid}); the quadrilateral
extraction itself is demonstrated on real ACPDS imagery in
Figures~\ref{fig:architecture} and~\ref{fig:detection}, and a clean
end-to-end SfM bird's-eye reconstruction remains future work
