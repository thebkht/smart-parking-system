...
% Distinguish captions from body text: smaller font with a bold "Figure N:" label.
...
% Float placement: let figures fill more of each page and pack tighter so
% they stay near their text instead of spilling onto end-of-document float
...
% Pack figures at the top of float-only pages instead of vertically
% justifying them (which inserts large gaps between stacked figures). All
% slack goes to the bottom; figures keep a fixed separation.
\makeatletter
\setlength{\@fptop}{0pt}
...
% Let columns end naturally instead of stretching inter-paragraph and
% section glue to justify the page bottom (which opens large vertical gaps
% on pages that carry a top figure).
\raggedbottom
...
documented here is what lifted it to the academic baseline. Code is
available at \url{https://github.com/thebkht/smart-parking-system}.
\end{abstract}
...
Figure~\ref{fig:architecture} gives the end-to-end architecture. Each
parking space's annotated quadrilateral is perspective-warped to a
$128\times128$ patch; a single compact YOLOv8n-cls model classifies every
patch as occupied or free; the per-spot results are temporally smoothed,
serialized to JSON, and posted to a FastAPI backend that serves the web
and mobile clients. The rest of this section describes each stage in turn.

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\linewidth]{figures/architecture.pdf}
\caption{End-to-end system architecture. A lot frame with quadrilateral
spot annotations is pooled into perspective-corrected $128\times128$
patches, classified by YOLOv8n-cls, temporally smoothed, and reported as
compact JSON to the FastAPI backend, which drives the web/mobile occupancy
maps and Find My Car. Only the JSON leaves the edge device.}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\linewidth]{figures/pooling.pdf}
...
\begin{figure*}[tp]
\centering
\includegraphics[width=0.75\linewidth]{figures/detection.jpg}
...
bird's-eye-view backdrop. Owner setup uploads photos to
\texttt{POST /layout}, which runs the Structure-from-Motion pass
\emph{server-side} to build the bird's-eye map and extract spot polygons;
if it cannot produce a usable layout it returns \texttt{422} and the app
falls back to manual polygon submission via \texttt{POST /map}. A
\texttt{PATCH /spots/\{id\}} route lets the owner correct spot labels after
generation. The \emph{Find My Car} flow adds
...
and its corner coordinates), with per-spot reference photos managed through
\texttt{POST /spots/\{id\}/references}. Owner-mutating routes can be
protected by optional bearer-token authentication
(\texttt{AUTH\_ENABLED}), which also scopes \emph{Find My Car} sessions to
their owner. The same published layout drives the edge
...
\begin{figure*}[tp]
\centering
\includegraphics[width=0.82\linewidth]{figures/yolov8n_results.png}
...
\begin{subfigure}[t]{0.55\textwidth}
  ...
  \caption{Normalized confusion matrix (validation split).}
\end{subfigure}\hfill
...
\end{subfigure}
\caption{YOLOv8n-cls qualitative results on the \emph{validation} split.
The confusion matrix (a) shows balanced performance across the
\emph{occupied} and \emph{free} classes (the held-out \emph{test}-split
matrix is reported separately in Figure~\ref{fig:cmtest}); (b) shows
predicted labels on warped validation patches.}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\linewidth]{figures/w11_val_test_gap.png}
...
\begin{figure*}[tp]
\centering
\includegraphics[width=0.78\linewidth]{figures/gen_confusion_test.png}
\caption{YOLOv8n-cls confusion matrix on the held-out \emph{test} split
(distinct from the validation-split matrix in
Figure~\ref{fig:qualitative}). Left: raw
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\linewidth]{figures/gen_pr_curve.png}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\linewidth]{figures/w11_per_weather.png}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\linewidth]{figures/w11_pooling_compare.png}
...
needs no training. SIFT-only is the final scope of this work: it
...
Two design choices drove the result. First, the smallest classifier won:
\texttt{yolov8n-cls} reaches $97.72\%$ test accuracy, ahead of
\texttt{yolov8s} ($96.91\%$) and \texttt{yolov8m} ($97.38\%$)
(Table~\ref{tab:testcompare}), while being roughly $9\times$ smaller in
parameters than the ResNet50 paper baseline. Capacity was not the
constraint, so the $2.83$\,MB nano checkpoint is the one we promoted and
deployed. Second, quadrilateral perspective-warp pooling beat the simpler
bounding-square crop by $+1.34$\,pp test accuracy ($97.72\%$ vs.\ $96.38\%$;
Table~\ref{tab:poolcompare}). Both effects reproduce on the held-out test
lots, and the pooling gain has a direct physical explanation: the warp
removes perspective distortion and excludes neighbouring vehicles that a
loose rectangle would include.

\subsection{Limitations}

The dominant limitation is \emph{patch quality}, not model capacity. The
test-split confusion matrix (Section~\ref{sec:results}, Table~\ref{tab:cmmetrics})
shows the residual error is concentrated in $26$ occupied$\rightarrow$free
misses out of $605$ occupied patches: these are predominantly partially
visible vehicles and thin border fragments introduced by the warp, exactly
the cases where larger variants do not help. The validation-to-test drop is
small ($-0.55$\,pp accuracy for the nano model) and consistent across
\texttt{n/s/m} (Table~\ref{tab:valtestgap}), which points to distribution
shift across ACPDS's unique-lot splits rather than overfitting. Robustness
degrades gracefully with light: accuracy falls monotonically from sunny to
low-light ($\sim$$2.3$\,pp; Table~\ref{tab:weather}), where the model
becomes conservative --- recall drops while precision stays high --- rather
than failing outright. Two scope limits remain. The \emph{Find My Car}
localization evaluation, while perfect on its set ($21/21$ top-1;
Section~\ref{sec:fmc-eval}), uses a small, single-condition (night-overhead)
query set, so the headline number should not be read as all-weather
accuracy. And label ambiguity in borderline patches caps the achievable
ceiling for any classifier on this data.

\subsection{Production considerations}

The deployed path has ample operating margin. The Core ML INT8 export runs
at $858$\,FPS ($1.2$\,ms per patch; Table~\ref{tab:speed}), about
$300\times$ the $\sim$$0.5$\,Hz reporting cadence, and the edge node sends
only compact occupancy JSON, a $\sim$$99.9\%$ bandwidth reduction versus
H.264 video (Section~\ref{sec:backend}). Temporal smoothing absorbs
per-frame flicker, and a 30-minute soak test on the live quadrilateral path
completed without crashes. On the product side, \emph{Find My Car} ships
SIFT-only: it is training-free, CPU-only, and sufficient for the evaluated
set; MobileNetV3 embeddings remain a documented optional upgrade for
broader lighting robustness rather than a requirement of this work.
...
The owner uploads photos of the lot to \texttt{POST /layout}; the backend
runs the SfM pass server-side to compute a bird's-eye-view layout and
extract quadrilateral spot polygons, persists it, and returns it for
preview. If SfM cannot build a usable layout the route returns
\texttt{422} and the app falls back to manual polygon submission. The owner
can then correct each spot's label inline (persisted via
\texttt{PATCH /spots/\{id\}}) before the layout drives every other screen.
This is the one-time configuration step that every other screen depends on.
The screenshot in Figure~\ref{fig:ui-owner} shows the web flow driving a
sample placeholder layout (\texttt{spot\_source: placeholder\_grid}); the
quadrilateral extraction itself is demonstrated on real ACPDS imagery in
Figures~\ref{fig:architecture} and~\ref{fig:detection}. The current SfM
stage uses ORB feature matching with homography blending; a
higher-fidelity bird's-eye reconstruction remains future work
(Section~\ref{sec:conclusion}).

\begin{figure}[tbp]
\centering
\fbox{\includegraphics[width=\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax]{figures/ui_owner_setup_layout.png}}
\caption{Owner setup (web): the photo-upload area and the resulting
12-spot layout panel (\emph{``Layout ready --- 12 spots detected''}),
ready to publish as the lot map. The spots shown are a sample placeholder
grid (\texttt{spot\_source: placeholder\_grid}) used to exercise the UI;
real quadrilateral extraction on ACPDS imagery is shown in
Figures~\ref{fig:architecture} and~\ref{fig:detection}.}
\label{fig:ui-owner}
\end{figure}
...
its current state, with a free / occupied / total summary. The mobile map
renders the same layout as coordinate-accurate SVG polygons with
pinch-to-zoom and pan. Counts are scoped to the active layout's spot IDs
(see Section~\ref{sec:casestudy}); in Figure~\ref{fig:ui-web-map} the
4 / 8 / 12 summary agrees with the colored spots.

\begin{figure}[tbp]
\centering
\fbox{\includegraphics[width=\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax]{figures/ui_web_map.png}}
\caption{Web live occupancy map: each published spot colored green (free)
or red (occupied), with free / occupied / total counters, a polling
control, and a live connection indicator.}
\label{fig:ui-web-map}
\end{figure}

\begin{figure}[tbp]
\centering
\fbox{\includegraphics[width=0.65\linewidth]{figures/ui_mobile_map.png}}
\caption{Mobile live occupancy map (SVG polygons with pinch-to-zoom and
pan). The 4 / 8 / 12 free / occupied / total summary and per-spot colors
mirror the web client of Figure~\ref{fig:ui-web-map} against the same
12-spot layout.}
\label{fig:ui-mobile-map}
\end{figure}

\subsection{Find My Car (mobile)}

On arrival the driver taps \emph{Park Here}; the app uploads a photo to
\texttt{POST /park}, which localizes it against the per-spot reference
photos (managed through \texttt{POST /spots/\{id\}/references}, with the
bundled sample set as fallback) and returns a \texttt{session\_id}.
...
\begin{figure}[tbp]
\centering
\fbox{\includegraphics[width=0.65\linewidth]{figures/ui_find_my_car.png}}
\caption{Find My Car (mobile): the driver's overhead photo is matched by
SIFT feature matching to the published layout (here \texttt{spot\_2},
similarity score 7.056), and the localized spot is highlighted in amber on
the map.}
\label{fig:ui-fmc}
\end{figure}
...
drives owner setup, map persistence, edge update, live status, the
park/find round-trip against the real localizer, owner label correction,
and per-spot reference management, and the clients carry contract tests for
the layout, occupancy-counting, label-correction, and park/find response
shapes.

Future work is narrow. Low-light recall is the weakest metric (93.5\,\%),
and luminance-aware augmentation is the natural remedy. Owner setup now runs
Structure-from-Motion on the uploaded photos server-side with a
manual-correction fallback; the remaining gap is reconstruction fidelity,
as the current ORB-based bird's-eye stage is coarse and a metric SfM or
learned homography would sharpen it. A longer soak test and backend-side
filtering of status payloads against the published layout (the hardening
from Section~\ref{sec:casestudy}) round out the remaining engineering work.
